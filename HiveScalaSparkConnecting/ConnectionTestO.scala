...
import org.apache.spark.sql.SparkSession // Must have hdfs running with yarn.
...
object ConnectionTestO{ // Object file name.
  def main(args: Array[String]): Unit = {             //Need hadoop, scala2.13, hive and jdk8 installed.
    ...
    val df2 = spark.read.format("csv").option("sep",",").load("hdfs://localhost:9000/user/hive/warehouse/Bev_BranchA.txt") // File location in running Hadoop
